...
with conn:
    ...
    # Plot 1: Кількість годин прослуховування на день та рівень депресії
    ...
    # Plot 2: Улюблені жанри тих, у кого рівень тривоги менше 5
    ...
    # Plot 3: Жанри та bpm музики для людей з середнім рівнем психічних захворювань більше 5, яким музика покращує психічний стан
    ax3 = fig.add_subplot(gs[1, :])
    cur.execute("SELECT * FROM genres_and_bpm_that_improve_mental_state")
    fav_genres, bpm_values = zip(*cur.fetchall())
    ax3.scatter(fav_genres, bpm_values)
    ax3.set_xlabel('Улюблений жанр')
    ax3.set_ylabel('BPM')
    ax3.set_title('Жанри та bpm музики для людей з середнім рівнем психічних захворювань більше 5, яким музика покращує психічний стан')
